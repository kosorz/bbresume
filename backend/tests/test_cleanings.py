...
class TestGetCleaning:
    async def test_get_cleaning_by_id(self, app: FastAPI, client: AsyncClient,
...
class TestUpdateCleaning:
    ...
    async def test_update_cleaning_with_valid_input(
        ...
        res = await client.put(app.url_path_for(
            "cleanings:update-cleaning-by-id", id=test_cleaning.id),
...
class TestDeleteCleaning:
    async def test_can_delete_cleaning_successfully(
        ...
        res = await client.delete(
            app.url_path_for("cleanings:delete-cleaning-by-id",
                             id=test_cleaning.id))
        assert res.status_code == HTTP_200_OK
    ...
    async def test_can_delete_cleaning_successfully(
            self, app: FastAPI, client: AsyncClient,
            test_cleaning: CleaningInDB, id: int, status_code: int) -> None:
        res = await client.delete(
